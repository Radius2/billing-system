...
export default function useDocumentsData({documentName, pageSize = 20, snackbarHandler, setDefaultCurrentMod, selectedRows, setErrMessage}) {
    ...
    const getElements = useCallback((page) => {
        setLoading(true);
        api.getHandbook(documentName, {page, page_size: pageSize, ...filterParams, ...sortParams})
            .then((resp) => {
    ...
    const addRow = useCallback((payload) => {
        api.addElementHandbook(documentName, payload)
            .then((resp) => {
                return api.getElementHandbook(documentName, resp.data.id)
            })
            .then(resp => {
    ...
    }, [documentName, setDefaultCurrentMod, lang, snackbarHandler])

    //обновить строку и получить перезаписанную сткоку из БД
    const updateRow = useCallback((payload, index) => {
        api.updElementHandbook(documentName, payload)
            .then((resp) => api.getElementHandbook(documentName, resp.data.id))
            .then(resp => {
            ...
            .catch(err => setErrMessage(err.message))
    }, [documentName, setDefaultCurrentMod, lang, snackbarHandler, selectedRows])


    //удалить строку локально и из БД
    const deleteRows = useCallback(() => {
            const ids = selectedRows.map((index) => data[index].id);
            api.delElementsHandbook(documentName, ids)
                .then(resp => {
                    setData(prev => {
        ...
        , [documentName, setDefaultCurrentMod, lang, snackbarHandler, selectedRows, console]
    )
    ...
    }, [documentName, filterParams, sortParams, getElements])
